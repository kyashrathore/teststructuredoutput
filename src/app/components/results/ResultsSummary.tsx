...
const ResultsSummary: React.FC<ResultsSummaryProps> = ({ results }) => {
  ...
  const totalCalls = results.reduce((sum, model) => sum + model.calls.length, 0);
  ...
  const sortedByTime = [...results].sort((a, b) => a.averageTimeMs - b.averageTimeMs);
  const fastestModel = sortedByTime.length > 0 ? sortedByTime[0] : null;
  const slowestModel = sortedByTime.length > 0 ? sortedByTime[sortedByTime.length - 1] : null;
  
  // Find model with highest success rate
  const sortedBySuccess = [...results].sort((a, b) => b.successRate - a.successRate);
  const mostReliableModel = sortedBySuccess.length > 0 ? sortedBySuccess[0] : null;
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
          <h4 className="text-sm font-medium text-slate-600 mb-3">Overall Performance</h4>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-2xl font-semibold text-slate-900 mt-1">{totalCalls}</p>
            </div>
        ...
        {/* Model Comparison */}
        <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
          <h4 className="text-sm font-medium text-slate-600 mb-3">Model Insights</h4>
                ...
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">
                      {fastestModel.modelName.split('/').pop()} ({formatTime(fastestModel.averageTimeMs)})
                    </p>
                  </div>
                </div>
              )}
              ...
              {mostReliableModel && mostReliableModel.successRate > 0 && (
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">
                      {mostReliableModel.modelName.split('/').pop()} ({(mostReliableModel.successRate * 100).toFixed(0)}%)
                    </p>
                  </div>
                </div>
              )}
      ...
      {/* Models Overview */}
      <div className="mt-6">
        <h4 className="text-sm font-medium text-slate-600 mb-3">Models Overview</h4>
          ...
          <table className="min-w-full divide-y divide-slate-200">
            <thead>
              <tr>
                ...
                <th className="px-3 py-2 text-right text-xs font-medium text-slate-500 uppercase tracking-wider">Avg Time</th>
                <th className="px-3 py-2 text-right text-xs font-medium text-slate-500 uppercase tracking-wider">Success Rate</th>
              </tr>
            </thead>
                  ...
                  <td className="px-3 py-2 whitespace-nowrap text-sm text-right">
                    <span 
                      className={`px-2 py-1 text-xs rounded-full ${
                        result.successRate >= 0.9 ? 'bg-emerald-100 text-emerald-800' :
                        result.successRate >= 0.7 ? 'bg-amber-100 text-amber-800' :
